...
- **Physical attackers:** prefer Adamant or Jolly when speed matters; avoid
  natures that lower Attack (see grid Atk column or alphabetical table).
- **Special attackers:** prefer Modest or Timid; avoid natures that lower Sp.
  Atk (see grid SpA column).
- **Defensive pivots / support:** Calm, Careful, Bold, and Impish are common
  when the Pokemon does not need the lowered stat.
- **Mixed or priority-sensitive sets:** verify the lowered stat is unused before
  committing; e.g. Naive trades bulk for speed on mixed attackers.
